...
export default async function HomePage() {
  const { data, dbReady } = await safeQuery(
    async (payload) => {
      const [franchises, players, sold, topPlayers, activity, settings] = await Promise.all([
        ...
        payload.find({ collection: 'activity', sort: '-createdAt', limit: 6 }),
    ...
    <div className="space-y-8">
      ...
      <GlassPanel strong className="relative overflow-hidden p-8 sm:p-10">
        ...
        <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/15 px-3 py-1 text-xs font-bold uppercase tracking-wider text-primary">
          <Lightning weight="fill" size={13} /> {data.season} · Live
        </span>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <StatTile label="Franchises" value={data.franchises} icon={UsersIcon} />
        <StatTile label="Player Pool" value={data.players} icon={IdentificationCard} />
        <StatTile label="Players Sold" value={data.sold} icon={Gavel} accent />
        <StatTile label="Season" value={data.season} icon={Trophy} />
      </div>
          ...
          {data.activity.length > 0 ? (
            <ul className="space-y-2">
              {data.activity.map((a) => (
                <li
                  key={a.id}
                  className="flex items-center gap-3 rounded-xl bg-foreground/5 px-3 py-2.5 text-sm"
                >
            ...
            <p className="py-8 text-center text-sm text-muted-foreground">
              No activity yet — the feed fills up as the auction, trades, and matches happen.
            </p>
          )}
        </GlassPanel>
      </div>
