...
import { Field, Select, NumberInput, TextInput, SubmitButton, MiniButton, type Option } from '@/components/commissioner/fields'
...
function GameRow({
  ...
  const aWin = !g.walkover && (g.scoreA ?? 0) > (g.scoreB ?? 0)
  const bWin = !g.walkover && (g.scoreB ?? 0) > (g.scoreA ?? 0)
  ...
  return (
    <GlassPanel className="p-4">
      <div className="flex items-center gap-3">
        ...
        <span className="skeuo-inset shrink-0 rounded-lg px-3 py-1 font-display text-lg font-black tabular-nums">
          {g.walkover ? 'W/O' : `${g.scoreA ?? 0} – ${g.scoreB ?? 0}`}
        </span>
        ...
        {canDelete && (
          ...
          >
            {pending ? <CircleNotch weight="bold" className="size-4 animate-spin" /> : <Trash weight="bold" className="size-4" />}
          </button>
        )}
      </div>
    </GlassPanel>
...
function Side({
  ...
  return (
    <div className={cn('min-w-0 flex-1', align === 'right' ? 'text-right' : 'text-left')}>
      ...
      {team && <p className="truncate text-xs text-muted-foreground">{team}</p>}
    </div>
  )
}
...
function LogGameForm({
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!walkover && (scoreA === '' || scoreB === '')) return toast.error('Enter both scores')
    start(async () => {
  ...
  if (participants.length < 2) {
    return (
      <GlassPanel className="p-5 text-sm text-muted-foreground">
        Add at least two participants to this tournament (in the commissioner panel) to start logging games.
      </GlassPanel>
    )
  }
            ...
            {(['1v1', '2v2'] as const).map((f) => (
              <button
                ...
                className={cn(
                  ...
                  format === f ? 'skeuo-btn text-foreground' : 'text-foreground/55 hover:text-foreground',
                )}
              >
                {f}
              </button>
        ...
        <div className="grid items-start gap-4 sm:grid-cols-[1fr_auto_1fr]">
          ...
          <div className="space-y-3">
            <Field label={format === '2v2' ? 'Side A — owners' : 'Side A — owner'}>
              <Select value={a1} onChange={(e) => setA1(e.target.value)} options={optionsExcluding(a2, b1, b2)} placeholder="— owner —" />
            </Field>
            {format === '2v2' && (
              <Select value={a2} onChange={(e) => setA2(e.target.value)} options={optionsExcluding(a1, b1, b2)} placeholder="— partner —" />
            )}
            <Field label="Playing as">
              <TextInput value={aTeam} onChange={(e) => setATeam(e.target.value)} placeholder="NBA team" />
            </Field>
            <Field label={walkover ? 'Score (optional)' : 'Score'}>
              <NumberInput min={0} value={scoreA} onChange={(e) => setScoreA(e.target.value)} placeholder={walkover ? '—' : '0'} className="text-center text-lg font-bold tabular-nums" />
            </Field>
          </div>
          ...
          <div className="space-y-3">
            <Field label={format === '2v2' ? 'Side B — owners' : 'Side B — owner'}>
              <Select value={b1} onChange={(e) => setB1(e.target.value)} options={optionsExcluding(a1, a2, b2)} placeholder="— owner —" />
            </Field>
            {format === '2v2' && (
              <Select value={b2} onChange={(e) => setB2(e.target.value)} options={optionsExcluding(a1, a2, b1)} placeholder="— partner —" />
            )}
            <Field label="Playing as">
              <TextInput value={bTeam} onChange={(e) => setBTeam(e.target.value)} placeholder="NBA team" />
            </Field>
            <Field label={walkover ? 'Score (optional)' : 'Score'}>
              <NumberInput min={0} value={scoreB} onChange={(e) => setScoreB(e.target.value)} placeholder={walkover ? '—' : '0'} className="text-center text-lg font-bold tabular-nums" />
            </Field>
          </div>
        </div>

        <label className="flex cursor-pointer items-center gap-2.5 text-sm">
          <input type="checkbox" checked={walkover} onChange={(e) => setWalkover(e.target.checked)} className="size-4 accent-primary" />
          <FlagBanner weight="fill" className={cn('size-4', walkover ? 'text-primary' : 'text-muted-foreground')} />
          Walkover (no game played — score optional)
        </label>
