...
/**
 * Goat head mark for the G.O.A.T Ring toggle. Vector from game-icons.net
 * ("Goat" by Skoll, CC BY 3.0) with the background plate removed so the
 * glyph follows currentColor like the Phosphor icons around it.
 */
export function GoatIcon({ size = 20, className }: { size?: number; className?: string }) {
  return (
    <svg
      viewBox="0 0 512 512"
      width={size}
...
/** Official 2K logo (public/logos/2k.svg) for the 2K Championship Ring toggle. */
export function TwoKLogo({ size = 20, className }: { size?: number; className?: string }) {
  return (
    // eslint-disable-next-line @next/next/no-img-element -- static SVG in /public; next/image blocks SVG without dangerouslyAllowSVG
    <img
      src="/logos/2k.svg"
      alt=""
      aria-hidden
